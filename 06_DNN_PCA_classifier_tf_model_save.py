...
pca = PCA(n_components=0.98, whiten=False) # svd_solver="randomized"
pca2 = PCA(n_components=0.99, whiten=False) #don't know why the number of features it's not equal in both cases. 

tr_features_pca = pca.fit_transform(tr_features)
ts_features_pca = pca2.fit_transform(ts_features)
print (tr_features_pca.shape[1])
print (ts_features_pca.shape[1])

file = open("dataset_PCA.txt", "w")
np.savetxt(file, (tr_features_pca))
file.close()
file = open("testset_PCA.txt", "w")
...
def create_model():
  model = tf.keras.models.Sequential([
    keras.layers.Dense(512, activation=tf.nn.relu, input_shape=(ins,)),
