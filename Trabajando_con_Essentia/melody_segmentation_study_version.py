...
def audio_segments_generator(audio):
    pitch_extractor = PredominantPitchMelodia(frameSize=fS, 
                                            hopSize=hS, 
                                            harmonicWeight=0.70,     #harmonic weighting parameter (weight decay ratio between two consequent harmonics, =1 for no decay)
                                            #magnitudeThreshold=40, #    spectral peak magnitude threshold (maximum allowed difference from the highest peak in dBs)
                                            #guessUnvoiced=True,
                                            minDuration=500, #    the minimum allowed contour duration [ms]
                                            numberHarmonics=20,
    ...
    # plt.show()
    # print('Amount of Segments:', sample_segments)
...
for audio_files in glob.glob( 'audio/5/' + "*.wav" ):
